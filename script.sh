...
if ! envsubst < ./automation-files2/01-workspaces-creation.yaml | kubectl apply -f -; then
...
if ! envsubst < ./automation-files2/06-virtualgroup-creation.yaml | kubectl apply -f -; then
...
if ! envsubst < ./automation-files2/08-workspaceroles-creation.yaml | kubectl apply -f -; then
...
if ! envsubst < ./automation-files2/09-virtualgroupkommanderworkspacerolebinding.yaml | kubectl apply -f -; then
...
if ! envsubst < ./automation-files2/10-virtualworkspacerolebinding.yaml | kubectl apply -f -; then
